        ...
        <>
          <div className="flex flex-col items-center gap-4 py-6">
            <button
              ...
              className={`grid h-40 w-40 place-items-center rounded-full text-6xl shadow-xl transition disabled:opacity-50 ${
                listening
            ...
            <div className="min-h-6 text-center text-sm text-slate-300">
              {listening && (interim ? <span className="italic">"{interim}"</span> : 'Listening…')}
            ...
            {voice && <div className="text-[10px] text-slate-500">voice: {voice.name}</div>}
          </div>

          {(() => {
            const lastAgent = [...messages].reverse().find(m => m.role === 'assistant')
            return lastAgent ? (
              <div className="rounded-xl border border-brand-700/40 bg-brand-900/20 p-3 text-center text-sm italic text-slate-200">
                "{lastAgent.content}"
              </div>
            ) : null
          })()}
